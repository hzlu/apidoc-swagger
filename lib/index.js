...
function createApidocSwagger(options) {
    ...
    try {
        ...
        if (app.options.parse !== true) {
            ...
            const extraDefinitions = readExtraSchemas(path.join(__dirname, '../../../apidoc-demo/schemas/'))
            api["swaggerData"] = JSON.stringify(apidocSwagger.toSwagger(apidocData, projectData, extraDefinitions));
...
function readExtraSchemas(path) {
    const files = fs.readdirSync(path);
    const definitions = {};
    for (const f of files) {
        const schema = JSON.parse(fs.readFileSync(path + f, 'utf8'));
